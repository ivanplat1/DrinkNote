...
export function getWeekStats(
  ...
  const totalVolumeMl = weekDrinks.reduce(
    (sum, d) => sum + d.volumeMl * (d.quantity ?? 1),
    0,
  );
...
export function getMonthStats(
  ...
  const totalVolumeMl = monthDrinks.reduce(
    (sum, d) => sum + d.volumeMl * (d.quantity ?? 1),
    0,
  );
...
export function getOverallStats(drinks: Drink[]): {
  ...
  const totalVolumeMl = drinks.reduce(
    (sum, d) => sum + d.volumeMl * (d.quantity ?? 1),
    0,
  );
...
export function getWeekDaysStats(
  drinks: Drink[],
  date: Date,
): Array<{ day: Date; units: number }> {
  const start = startOfWeek(date);
...
export function getLastNWeeks(
  drinks: Drink[],
  n: number,
): Array<{ weekStart: Date; stats: ReturnType<typeof getWeekStats> }> {
  const result: Array<{
    weekStart: Date;
...
export function getLastNMonths(
  drinks: Drink[],
  n: number,
): Array<{ month: Date; stats: ReturnType<typeof getMonthStats> }> {
  const result: Array<{
    month: Date;
...
export function getBeverageTypeStats(drinks: Drink[]): Array<{
  ...
  const typeMap = new Map<
    Drink["beverageType"],
    { units: number; volume: number; spent: number; count: number }
  >();

  drinks.forEach((drink) => {
    const existing = typeMap.get(drink.beverageType) || {
      units: 0,
      volume: 0,
      spent: 0,
      count: 0,
  ...
  return Array.from(typeMap.entries())
    .map(([type, data]) => ({
      ...
      percentage:
        totalUnits > 0 ? Math.round((data.units / totalUnits) * 1000) / 10 : 0,
      count: data.count,
    }))
...
export function getWeekdayStats(drinks: Drink[]): Array<{
  ...
  for (let i = 0; i < 7; i++) {
    ...
    result.push({
      ...
      totalUnits: Math.round(data.units * 100) / 100,
      averageUnits:
        daysCount > 0 ? Math.round((data.units / daysCount) * 100) / 100 : 0,
      daysCount,
    });
  }
...
export function getRecords(drinks: Drink[]): {
  ...
  if (allDates.length > 0) {
    ...
    const daysSinceLastRecord = Math.floor(
      (todayDate.getTime() - lastRecordDate.getTime()) / (1000 * 60 * 60 * 24),
    );
...
export function comparePeriods(
  ...
  const period1Drinks = drinks.filter(
    (d) => d.dateISO >= period1ISOStart && d.dateISO <= period1ISOEnd,
  );
  const period2Drinks = drinks.filter(
    (d) => d.dateISO >= period2ISOStart && d.dateISO <= period2ISOEnd,
  );

  const period1Units = period1Drinks.reduce(
    (sum, d) => sum + d.standardUnits,
    0,
  );
  const period2Units = period2Drinks.reduce(
    (sum, d) => sum + d.standardUnits,
    0,
  );
  const period1Spent = period1Drinks.reduce(
    (sum, d) => sum + (d.price ?? 0),
    0,
  );
  const period2Spent = period2Drinks.reduce(
    (sum, d) => sum + (d.price ?? 0),
    0,
  );
  ...
  const unitsPercent =
    period1Units > 0 ? (unitsChange / period1Units) * 100 : 0;
  const spentChange = period2Spent - period1Spent;
  const spentPercent =
    period1Spent > 0 ? (spentChange / period1Spent) * 100 : 0;
...
export function getDetailedWeekdayAnalytics(
  drinks: Drink[],
  weekdayNames?: string[],
): Array<{
  weekday: number;
  ...
  const totalUnits = drinks.reduce((sum, d) => sum + d.standardUnits, 0);
  const WEEKDAY_NAMES = weekdayNames ?? [
    "Понедельник",
    "Вторник",
    "Среда",
    "Четверг",
  ...
  return weekdayStats.map((stat, index) => {
    ...
    return {
      ...
      percentageOfTotal:
        totalUnits > 0
          ? Math.round((stat.totalUnits / totalUnits) * 1000) / 10
          : 0,
      trend,
    };
  });
...
export function compareCurrentWeekWithPrevious(
  drinks: Drink[],
): ReturnType<typeof comparePeriods> {
  const today = new Date();
...
export function compareCurrentWeekWithPreviousAdjusted(
  drinks: Drink[],
): ReturnType<typeof comparePeriods> {
  const today = new Date();
...
export function compareCurrentMonthWithPrevious(
  drinks: Drink[],
): ReturnType<typeof comparePeriods> {
  const today = new Date();
...
export function compareCurrentMonthWithPreviousAdjusted(
  drinks: Drink[],
): ReturnType<typeof comparePeriods> {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
export function compareCurrentYearWithPrevious(
  drinks: Drink[],
): ReturnType<typeof comparePeriods> {
  const today = new Date();
  ...
  const previousYearEnd = new Date(
    ...
    today.getMonth(),
    today.getDate(),
  ); // тот же день и месяц в прошлом году
