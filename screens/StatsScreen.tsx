...
export default function StatsScreen() {
  ...
  const overallStats = useMemo(
    () => getOverallStats(filteredDrinks),
    [filteredDrinks],
  );
  const weekStats = useMemo(
    () => getWeekStats(filteredDrinks, selectedDate),
    [filteredDrinks, selectedDate],
  );
  const monthStats = useMemo(
    () => getMonthStats(filteredDrinks, selectedDate),
    [filteredDrinks, selectedDate],
  );
  ...
  const beverageTypeStats = useMemo(
    () => getBeverageTypeStats(filteredDrinks),
    [filteredDrinks],
  );
  const weekdayStats = useMemo(
    () => getWeekdayStats(filteredDrinks),
    [filteredDrinks],
  );
  const records = useMemo(() => getRecords(filteredDrinks), [filteredDrinks]);
  const topDrinks = useMemo(
    () => getTopDrinks(filteredDrinks, 5),
    [filteredDrinks],
  );
  ...
  const chartYAxisValues = useMemo(() => {
    ...
    return [maxChartValue, step * 3, step * 2, step, 0].map(
  ...
  const weekdayYAxisValues = useMemo(() => {
    ...
    return [maxWeekdayValue, step * 3, step * 2, step, 0].map(
      (v) => Math.round(v * 10) / 10,
    );
  }, [maxWeekdayValue]);
        ...
        >
          <Text
            style={[
              ...
              activeTab === "basic" && styles.parentTabTextActive,
              {
                color:
                  activeTab === "basic" ? colors.primary : colors.textSecondary,
              },
            ]}
          >
        ...
        >
          <View style={styles.parentTabButtonContent}>
            <Text
              style={[
                styles.parentTabText,
                activeTab === "advanced" && styles.parentTabTextActive,
                {
                  color:
                    activeTab === "advanced"
                      ? colors.primary
        ...
        !isPremium ? (
          <View style={styles.lockedContainer}>
            ...
            <Text style={styles.lockedTitle}>
              {t("premium.features.advancedStatsTitle")}
            </Text>
            <Text style={styles.lockedText}>{t("premium.notAvailable")}</Text>
            <TouchableOpacity
              style={[
        ...
        >
          ...
          {!isPremium && allDrinks.length > filteredDrinks.length && (
            <View style={styles.limitWarning}>
              ...
              >
                ...
                />
                <Text style={styles.limitWarningButtonText}>
                  {t("statsScreen.premiumShort")}
                </Text>
              </TouchableOpacity>
            </View>
          )}
          ...
          >
            <TouchableOpacity
              style={[
                styles.periodButton,
                period === "overall" && styles.periodButtonActive,
                period === "overall" && { backgroundColor: colors.primary },
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  period === "overall" && styles.periodButtonTextActive,
                  {
                    color: period === "overall" ? "#fff" : colors.textSecondary,
                  },
                ]}
                numberOfLines={1}
              >
          ...
          >
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: colors.primary }]}>
                  {currentStats.totalUnits.toFixed(2)}
                </Text>
                <Text
                  style={[styles.statLabel, { color: colors.textSecondary }]}
                >
                  {t("statsScreen.standardUnits")}
                </Text>
                <Text
                  style={[styles.statSubLabel, { color: colors.textTertiary }]}
                >
                  {tf("stats.gramsAlcohol", {
                    g: (currentStats.totalUnits * 10).toFixed(1),
                  })}
                </Text>
              </View>
              <View style={styles.statItem}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: colors.primary }]}>
                  {period === "overall"
                    ? (currentStats as typeof overallStats).totalDays
                    : currentStats.daysWithDrinks}
                </Text>
                <Text
                  style={[styles.statLabel, { color: colors.textSecondary }]}
                >
                  {t("stats.daysWithEntries")}
                </Text>
                {period !== "overall" && (
                  <Text
                    style={[
                  ...
                  >
                    {period === "week"
                      ? tf("stats.pctOfWeek", {
                          pct: Math.round(
                            ((currentStats.daysWithDrinks || 0) / 7) * 100,
                          ),
                        })
                      : period === "month"
                        ? tf("stats.pctOfMonth", {
                            pct: Math.round(
                              ((currentStats.daysWithDrinks || 0) /
                                new Date(
              ...
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: colors.primary }]}>
                  {currentStats.averagePerDay.toFixed(2)}
                </Text>
                <Text
                  style={[styles.statLabel, { color: colors.textSecondary }]}
                >
                ...
                >
                  {tf("stats.gramsAlcohol", {
                    g: (currentStats.averagePerDay * 10).toFixed(1),
                  })}
                </Text>
              </View>
            </View>
                ...
                <View style={styles.statItem}>
                  <Text style={[styles.statValue, { color: colors.primary }]}>
                    {currentStats.totalSpent > 0
                      ? formatPriceValueOnly(currentStats.totalSpent)
                      : "—"}
                  </Text>
                  <Text
                    style={[styles.statLabel, { color: colors.textSecondary }]}
            ...
            >
              <Text style={[styles.chartTitle, { color: colors.text }]}>
                {tf("stats.trendTitle", {
                  unit:
                    period === "week"
                      ? t("stats.trendUnitWeek")
                      : t("stats.trendUnitMonths"),
                })}
              </Text>
                ...
                <View style={styles.chartContainer}>
                  {chartData.map((item, index) => {
                    const height =
                      period === "week"
                        ? maxChartValue > 0
                          ? (item.units / maxChartValue) * 100
                          : 0
                        : maxChartValue > 0
                          ? (item.stats.totalUnits / maxChartValue) * 100
                          : 0;
                    const value =
                      period === "week" ? item.units : item.stats.totalUnits;
                    return (
                      <View key={index} style={styles.chartBarContainer}>
                        ...
                        >
                          {period === "week"
                            ? WEEKDAY_SHORT[index]
                            : MONTH_SHORT[item.month.getMonth()]}
                        </Text>
                      </View>
                    );
                  })}
          ...
          {period === "overall" &&
            overallStats.firstDate &&
            overallStats.lastDate && (
              <View
                style={[
                  styles.infoCard,
                  { backgroundColor: colors.backgroundCard },
              ...
              >
                ...
                >
                  {t("stats.period")}:{" "}
                  {new Date(overallStats.firstDate).toLocaleDateString(
                    localeTag,
                  )}{" "}
                  -{" "}
                  {new Date(overallStats.lastDate).toLocaleDateString(
                    localeTag,
                  )}
                  {!isPremium && allDrinks.length > filteredDrinks.length && (
                    <Text
              ...
              <View style={styles.chartWithAxis}>
                ...
                <View style={styles.chartContainer}>
                  {weekdayStats.map((item) => {
                    const height =
                      maxWeekdayValue > 0
                        ? (item.averageUnits / maxWeekdayValue) * 100
                        : 0;
                    return (
                          ...
                          >
                            {height > 0 && (
                              <>
                                ...
                                {height > 15 && (
                                  <Text style={styles.chartValueLabel}>
                                    {item.averageUnits.toFixed(
                                      item.averageUnits >= 10 ? 0 : 1,
                                    )}
                                  </Text>
                                )}
                              </>
            ...
            >
              ...
              <View style={styles.recordsRow}>
                ...
                >
                  ...
                  >
                    {t("statsScreen.heaviestDay")}
                  </Text>
                  <Text style={[styles.recordValue, { color: colors.primary }]}>
                    {records.heaviestDay.units.toFixed(2)}{" "}
                    {t("advancedStats.unitsShort")}
                  </Text>
                  <Text
                    style={[
                      styles.recordSubValue,
                      { color: colors.textSecondary },
                    ]}
                  >
                    {tf("stats.gramsAlcohol", {
                      g: (records.heaviestDay.units * 10).toFixed(1),
                    })}
                  </Text>
                  <Text
                    style={[styles.recordDate, { color: colors.textTertiary }]}
                  >
                    {new Date(records.heaviestDay.date).toLocaleDateString(
                      localeTag,
                ...
                {records.currentStreak > 0 && (
                  ...
                  >
                    ...
                    >
                      {t("statsScreen.alcoholFreeDays")}
                    </Text>
                    <Text
                      style={[styles.recordValue, { color: colors.primary }]}
                    >
              ...
              {records.longestStreak > 0 && (
                <View style={styles.recordsRow}>
                  ...
                  >
                    ...
                    >
                      {t("statsScreen.bestStreak")}
                    </Text>
                    <Text
                      style={[styles.recordValue, { color: colors.primary }]}
                    >
                      {records.longestStreak} {t("advancedStats.daysShort")}
                    </Text>
                  </View>
                </View>
              )}
              {isPremium && streakGoal != null && streakGoal > 0 && (
                ...
                >
                  ...
                  >
                    {tf("statsScreen.goal", {
                      days: `${streakGoal} ${t("advancedStats.daysShort")}`,
                    })}
                  </Text>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    ...
                    <Text
                      style={[
                        styles.recordValue,
                        { color: colors.primary, fontSize: 14, marginLeft: 8 },
                      ]}
                    >
                      {records.currentStreak} / {streakGoal}
                    </Text>
                ...
                return (
                  ...
                  >
                    ...
                    <View style={styles.topDrinkInfo}>
                      <Text
                        style={[styles.topDrinkName, { color: colors.text }]}
                      >
                        {drink.name}
                      </Text>
                      <Text
                        style={[
                          styles.topDrinkDetails,
                          { color: colors.textSecondary },
                        ]}
                      >
                        {drink.count} × ·{" "}
                        ...
                        · {drink.totalUnits.toFixed(1)}{" "}
                        {t("advancedStats.unitsShort")} (
                        {Math.round(drink.totalUnits * 10)} {t("common.gShort")}
                        )
                      </Text>
                    </View>
                    <View
                      style={[
                        styles.topDrinkTypeDot,
                        { backgroundColor: typeColors.main },
                      ]}
                    />
                  </View>
