      ...
      tabBarSelectors.forEach(selector => {
        try {
          ...
          tabBars.forEach(tabBar => {
            ...
            // Важно: получаем элементы в порядке их появления в DOM
            const clickableElements = Array.from(tabBar.querySelectorAll('a[role="tab"], button[role="tab"], a[href], button'));
            
            // Сортируем элементы по их позиции в DOM (слева направо)
            ...
            clickableElements.forEach((element, index) => {
              ...
              // Используем одинаковый z-index для всех элементов, чтобы не было перекрытий
              ...
              // Убеждаемся, что элемент не перекрыт другими элементами
              // Проверяем, что элемент не перекрыт по горизонтали
              const rect = element.getBoundingClientRect();
              
              // Убеждаемся, что элемент имеет правильную ширину и не перекрыт
              if (rect.width === 0 || rect.height === 0) {
                element.style.setProperty('min-width', '60px', 'important');
                element.style.setProperty('min-height', '60px', 'important');
              }
              
              // Убеждаемся, что клики не блокируются
              element.style.setProperty('touch-action', 'manipulation', 'important');
              
              // Убеждаемся, что элемент не имеет overflow: hidden, который может скрывать клики
              if (computedStyle.overflow === 'hidden') {
                element.style.setProperty('overflow', 'visible', 'important');
              }
              
              // Убеждаемся, что родительские элементы не блокируют клики
              let parent = element.parentElement;
              let depth = 0;
              while (parent && parent !== tabBar && depth < 3) {
