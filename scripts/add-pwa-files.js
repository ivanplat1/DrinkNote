  ...
  if (!html.includes('safe-area-inset')) {
    ...
    <style>
      ...
      @media (display-mode: standalone) {
        ...
        /* УБИРАЕМ агрессивные стили position: fixed для таб-бара, чтобы не нарушать работу React Navigation */
        /* React Navigation сам управляет позиционированием таб-бара */
        nav[role="tablist"],
        [data-testid="tab-bar"],
        .tab-bar {
          /* Применяем только padding для safe area, не трогаем position */
        ...
        /* УБИРАЕМ агрессивные стили для дочерних элементов таб-бара */
        ...
        /* УБИРАЕМ агрессивные стили для кнопок таб-бара, чтобы не нарушать работу React Navigation */
        /* Оставляем только минимальные стили для safe area */
        nav[role="tablist"] a,
...
<script>
  ...
  (function() {
    ...
    // УПРОЩЕННАЯ ВЕРСИЯ: минимальные изменения, чтобы не нарушать работу React Navigation
      ...
      tabBarSelectors.forEach(selector => {
        try {
          ...
          tabBars.forEach(tabBar => {
            // Минимальные изменения: только убеждаемся, что таб-бар не заблокирован
            const computedStyle = window.getComputedStyle(tabBar);
            if (computedStyle.pointerEvents === 'none') {
              tabBar.style.setProperty('pointer-events', 'auto', 'important');
            }
            
            // Ищем все кликабельные элементы внутри таб-бара
            const clickableElements = tabBar.querySelectorAll('a[role="tab"], button[role="tab"], a[href]');
            
            clickableElements.forEach((element) => {
              // Минимальные изменения: только убеждаемся, что элемент не заблокирован
    ...
    function applySafeAreaToTabBar() {
      ...
      // ВАЖНО: НЕ применяем position: fixed через JavaScript, так как это нарушает работу React Navigation
      // Вместо этого используем только CSS стили из safeAreaCSS выше
      tabBarSelectors.forEach(selector => {
        try {
          const elements = document.querySelectorAll(selector);
          elements.forEach(element => {
            // НЕ применяем position: fixed через JavaScript - это делается через CSS
            // Применяем только padding для safe area
      ...
      // УБИРАЕМ вызовы fixTabBarLabels и fixTabBarClicks из applySafeAreaToTabBar
      // чтобы не нарушать работу React Navigation при каждом изменении
    }
    ...
    function initObservers() {
      ...
      // УБИРАЕМ MutationObserver для таб-бара, так как он может нарушать работу React Navigation
      // Применяем исправления только при загрузке страницы
    ...
    // Применяем после загрузки DOM - УПРОЩЕННАЯ ВЕРСИЯ
    // Убираем частые вызовы fixTabBarLabels и fixTabBarClicks, чтобы не нарушать работу React Navigation
