...
<script>
  ...
  (function() {
    ...
    function applySafeAreaToTabBar() {
      // Ищем таб-бар React Navigation (он обычно имеет role="tablist" или находится внизу экрана)
      const tabBarSelectors = [
        'nav[role="tablist"]',
        '[data-testid="tab-bar"]',
        '.tab-bar',
        // React Navigation Bottom Tabs обычно имеет position: fixed и bottom: 0
      ...
      tabBarSelectors.forEach(selector => {
        ...
        elements.forEach(el => {
          ...
          if (style.position === 'fixed' && (style.bottom === '0px' || style.bottom === '0')) {
            ...
            el.style.paddingBottom = \`calc(\${safeAreaBottom} + \${el.style.paddingBottom || '0px'})\`;
      ...
      const tabBarObserver = new MutationObserver(() => {
        tabBarSelectors.forEach(selector => {
          ...
          elements.forEach(el => {
            const style = window.getComputedStyle(el);
            if (style.position === 'fixed' && (style.bottom === '0px' || style.bottom === '0')) {
              if (!el.dataset.safeAreaApplied) {
                const safeAreaBottom = getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-bottom') || 'env(safe-area-inset-bottom)';
                el.style.paddingBottom = \`calc(\${safeAreaBottom} + \${el.style.paddingBottom || '0px'})\`;
                el.style.marginBottom = '0';
                el.dataset.safeAreaApplied = 'true';
              }
            }
          });
        });
